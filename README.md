...
4. **工具栏 (卡片级)**
    ...
    - **格式刷 (智能继承引擎)：** 吸取选中卡片内部的所有信息与排版（会自动剥离旧的历史生成图片/视频）。
        ...
        - **画在现有卡片 (覆盖)：** 仅覆盖目标的排版比例和节点绑定规则，**智能保留**目标原有的具体参数值，绝不冲掉你辛苦输入的提示词！如果目标卡片坑位不足而新生了模块，新顺延的模块则会带上格式刷的初始参数值。
...
3. **工具栏 (模块级)**
    ...
    - **输入模块专属：**
        ...
        - **拾取节点（准星图标）的左右键区分：** * **鼠标左键点击节点**：**替换**当前关联的节点，并自动清空之前绑定的参数，恢复初始状态。
...
- **视频组件：** 纯手工打造的无缝媒体播放器。
    ...
    - **其它操作：** 单击画面播放/暂停；提供独立倍速输入框；内置点击式极简进度条。
- **音频组件：**
