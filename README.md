...
3. **工具栏 (模块级)**
    ...
    - **输出模块专属：**
        ...
        - **历史参数联动回填（安全模式）：** 切换输出历史时，会同步恢复当次生成对应的输入模块参数（包含标题、参数值、关联节点、绑定参数）；缺失模块会自动补齐，不会自动删除你当前多出来的输入模块。
    - **通用工具：**
        - **重置：** 一键清空绑定参数和设置。
...
4. **内容**
    ...
    - **输出模块：** 选中模块后，按键盘的 **`←` 和 `→` 左右方向键** 切换显示历史生成记录对比；切换时会同步回填该条历史对应的输入模块参数。
5. **右键菜单**
    - **内容(输出模块专属)：**
        - **下载 / 下载全部生成记录：** 提取媒体保存至本地。
        - **移除：** 仅在当前模块内移除该记录（不影响本地文件）；对应的输入模块快照会同步删除。
        - **清除所有生成记录：** 清除当前模块的所有生成记录（不影响本地文件）；对应的输入模块快照也会一并清空。
...
- **输入模块快照冻结（历史参数回放基础）：**
    - 点击运行的那一刻，系统会冻结当前任务卡片里“已绑定节点参数”的输入模块。
...
- **历史记录容量上限**：限制单个输出模块最多保存的生成记录数。超出后会自动剔除最旧的记录以防止内存溢出；对应的输入模块快照也会同步剔除，避免历史与参数错位。
