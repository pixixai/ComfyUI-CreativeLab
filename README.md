...
# 🚀 ComfyUI ShellLink 插件使用手册

ShellLink 是一个为 ComfyUI 打造的沉浸式、卡片化任务管理面板。它通过完全解耦的架构，让你在不改变底层连线的情况下，像使用专业非编软件一样管理、克隆、批处理多个分镜和参数组合。
...
4. **工具栏**
    *   **克隆：** 复制选中的卡片——包括内部所有模块、模块工具栏参数（但不会包含“输出模块”的生成记录）。多选后可批量克隆，克隆体会紧随其后生成。
    *   **格式刷：** 把选中卡片的内部所有信息，完整的克隆到目标卡片（点击其它卡片完全覆盖替换属性；点击面板背景的空白间隙处，会直接在点击的位置把卡片像盖章一样复制过去）。
...
3. **工具栏**
    ...
    *   **通用工具：**
        *   **重置：** 一键清空绑定参数和设置。
        *   **同步参数：** 将当前参数一键同步给其他所有卡片中“标题相同”的模块。
        *   **克隆 / 格式刷：** 与卡片级逻辑一致，作用于单个模块属性或空间插入。
...
5. **右键菜单 (输出模块专属)**
    *   **内容：**
        ...
        *   **清除：** 清除当前模块的所有生成记录（不影响本地文件）。
    *   **模块：**
        *   **选择相同模块：** 将所有卡片中标题相同的模块一并选中。
        *   **删除相同模块：** 危险操作，一键物理删除所有同名模块。
        *   **批量向后移动 / 批量向前移动：** 跨卡片统一推移模块位置。
...
*   **收集整理**
    *   **移动到子文件夹：** 物理移动杂乱的文件到当前工作流专属子文件夹，并重命名，自动刷新地址防裂图。
    *   **复制到子文件夹：** 复制一份归档，不影响原路径。
...
*   **创建配置锚点：** 在画布上生成隐形的 `ShellLinkSystemConfig` 节点，将面板数据转化为 JSON 存入，随工作流 `Ctrl+S` 永久保存。
*   **清理失效记录：** 纯前端高并发探针，瞬间扫描所有历史记录，自动从内存中将本地已被删除的图片“死链(404)”剔除。
*   **重新同步记录（适用于 PS 外部修图）：** 它相当于一个强制刷新按钮。当你在本地硬盘用 Photoshop 修改了插件生成的图片覆盖保存后，浏览器因为缓存不会更新。点击此按钮，系统会强行下发时间戳（Cache-Busting）绕过缓存，瞬间在面板上加载修改后的最新画面。
...
*   **视频组件：** 纯手工打造的无缝媒体播放器。
    *   **视频进度调节（右键滑动）：** 在视频画面上，不需要点击底部的细小进度条，直接**按住鼠标右键并左右滑动**，即可精准调节进度，左下角的时间码会变成绿色的 `分:秒:帧 (MM:SS:FF)` 格式。
    *   **左大右小的抽屉式音量条：** 鼠标悬停在喇叭图标上会向左展开音量滑杆。向左拉（远离图标）是增大音量，向右推是减小音量。在静音状态下拖动滑块会自动解除静音。如果系统探测到视频**没有音轨**，静音按钮会自动变成不可交互的灰色。
    *   **其它操作：** 单击画面播放/暂停；提供独立倍速输入框。
*   **音频组件：**
...
*   **文件保存流程 (事后截胡转移法)：** 无论预览还是保存节点出图，前端瞬间拦截，呼叫后端 API 将文件复制并重命名到专属的 `output/ShellLink` 文件夹，实现所见即所得的永久归档。
